...
for subject in tqdm(os.listdir(path_annotations), total=len(os.listdir(path_annotations))):
    ...
    data = data[data.label==1] # keep only annotated sampels
    data = data.drop(columns=['turning1-l', 'turning2-l', 'turning1-r', 'turning2-r', 'stand-to-sit', 'walking', 'label'])

    # generate random subject id and file id
    ...
    # save subject data
    data.to_parquet(os.path.join(output_path, "fog@home_preprocessed", id + ".parquet"), index=False, compression='snappy')
